...
test_name = "k_fold_performance_analysis_cars_normalized"
...
print(X.isnull().sum())
# 6 rows with missing values in the 'horsepower' column, are removed
...
ldglm_with_nu_d_parameter_space = lidglm_create_params(families = ["gaussian"], links = ["identity"],
                                                                       ...
                                                                       nums_batches = [1],
                                                                       use_nu_d=[True, False],
                                                                       lipschitz_consts_2 = [1.4, 1.6, 1.99], 
                                                                       widths_2 = [4,8],
                                                                       depths_2 = [2,3], 
                                                                       num_blocks_2 = [1, 2],
...
sddr_param_space = sddr_create_params(families = ["Normal"],epochs = [5000], 
                                                                       depths=[2,3], depths_2=[2],
                                                                       widths=[16, 32, 64], dfs=[0], output_shape=[8, 16], widths_2 =[6, 16], dfs_2 = [0], output_shape_2 = [8],
                                                                       batch_size= [128, 256], optimizers = ["adam"], lrs= [1e-1, 1e-2, 1e-3], weight_decays=[0., 1e-5, 1e-4],
                                                                       dropout_rates=[0., 0.05,0.1], early_stop_epochs=[20], orthog = [False, True])
...
elastic_net_metrics_dataframe = k_fold_performance_analysis_smglm(covariates, target, elastic_net_param_space, metrics, temp_dir,  k=5, test_name=test_name + "_elastic_aki", number_of_trials_per_split=128, mode = "continuous", regularization="elastic_net")
...
lidglm_with_nu_d_metrics_dataframe = k_fold_performance_analysis_lidglm(covariates, target, ldglm_with_nu_d_parameter_space, metrics, temp_dir,  k=5, test_name="k_fold_performance_analysis_cars"+"_ldglm_with_nu_d", number_of_trials_per_split=256, mode = "continuous")
...
lidglm_metrics_dataframe = k_fold_performance_analysis_lidglm(covariates, target, ldglm_parameter_space, metrics, temp_dir,  k=5, test_name="k_fold_performance_analysis_cars"+"_ldglm", number_of_trials_per_split=256, mode = "continuous")
